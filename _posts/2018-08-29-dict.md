...
>>> d_proxy[1] # d의 항목을 d_proxy를 통해서 볼수있다
'A'
...
> setcomps는 dict comprehension와 같이 2.7에서 추가

 - 간단한 예제로 유니코드 이름에 SIGN word가 있는 문자 셋 생성
{% highlight python %}
>>> from unicodedata import name
#  32~256 캐릭터에 'SIGN' 이름이있는 원소로 set 생성
...
- my_dict[search_key]로 벨류를 갖고오는 경우
  1.  hash(search_key) 호출
  2. 키에 hash value 얻어옴
...
1. \__hash__() 메서드를 통해 hash() 함수를 지원, 이메서드는 항상 생명주기동안 같은 벨류를 리턴
2. __eq__() 메서드를 통해서 이퀴 비교 지원
